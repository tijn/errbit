...
describe ExceptionFilter do
  ...
  let(:attributes) { Fabricate.attributes_for(:empty_filter) }
  let!(:filter) { ExceptionFilter.new attributes }
  ...
  context 'can belong to an application' do
    let(:exception_filter) { Fabricate(:exception_filter) }
